package de.hglabor.bunkers.listener.claims
...
object PlayerClaimListeners {
    fun enable() {
        listen<PlayerEnterClaimEvent> {
            val team = Core.teamManager.teamByName(it.claim.name) as? BunkersTeam
            val claimColor = team?.teamColor ?: KColors.WHITE

            it.player.sendMsg {
                text("Now entering: ") { color = KColors.GRAY }
                text(it.claim.name) { color = claimColor }
            }
        }

        listen<PlayerLeaveClaimEvent> {
            val team = Core.teamManager.teamByName(it.claim.name) as? BunkersTeam
            val claimColor = team?.teamColor ?: KColors.WHITE

            it.player.sendMsg {
                text("Now leaving: ") { color = KColors.GRAY }
                text(it.claim.name) { color = claimColor }
            }
        }
    ...
    private fun cancelEvent(event: Cancellable, player: Player, location: Location) {
        ...
        val team = Core.teamManager.teamByName(claim.name) as? BunkersTeam
        val claimColor = team?.teamColor ?: KColors.WHITE
        ...
        if (event is BlockEvent || event is PlayerBucketEvent) {
            if (shouldCancelBlockEvent(player, claim)) {
                ...
                player.sendMsg {
                    ...
                    text("${claim.name}'s ") { color = claimColor }
    ...
    private fun shouldCancelBlockEvent(player: Player, claim: IClaim): Boolean {
        ...
        if (GameManager.pvpStyleManager.style == ClassicStyle) {
            if (claim is WarzoneClaim) {
                return when (player.inventory.itemInMainHand.type) {
                    Material.LAVA_BUCKET,
                    Material.WATER_BUCKET,
                    Material.COBWEB -> false
        ...
        if (clickedBlock != null) {
            when {
                MaterialTags.AXES.isTagged(itemInMainHand) || itemInMainHand.type == Material.HONEYCOMB -> {
                    when {
                        MaterialTags.COPPER_BLOCKS.isTagged(clickedBlock.type) || MaterialSetTag.LOGS.isTagged(
                            clickedBlock.type
                        ) -> {
                            return true
                        }
